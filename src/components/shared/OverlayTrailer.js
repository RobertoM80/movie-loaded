...
import { imageUrlBase, image_sizes } from '../../config/actions/movieActions';
import { getExerpt } from '../../config/reusableFunctions';
import { connect } from 'react-redux';

class OverlayTrailer extends Component {
    iframe(html) {
        return { __html: html }
    }
                    ...
                    <div className="col-sm-5">
                        { this.props.most_popular_movies.map((movie, index) => {
                            if (movie.title === this.props.current_youtube_trailer_movie_title) return (
                                <div key={ index + 1 } className="overlay-trailer-infos">
                                    <img src={ imageUrlBase + image_sizes[4] + movie.backdrop_path } alt="poster image" className="rounded poster-image img-fluid d-none d-lg-block" />
...
export default connect(mapStateToProps, null)(OverlayTrailer);